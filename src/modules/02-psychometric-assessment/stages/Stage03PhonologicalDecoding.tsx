...
export function Stage03PhonologicalDecoding({ onStageComplete }: Stage03Props) {
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (evalResult.shouldEscalateToHard) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_03_ITEMS, 'hard'));
      setActiveTier('hard');
    } else if (evalResult.shouldBranchToEasy) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_03_ITEMS, 'easy'));
      setActiveTier('easy');
    } else if (queue.length - (activeItemIndex + 1) + evalResult.nextValidScoredCount < DEFAULT_ADAPTIVE_CONFIG.minScoredItems) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_03_ITEMS, activeTier === 'hard' ? 'easy' : 'hard'));
    }
