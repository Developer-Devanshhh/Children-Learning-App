/**
 * Stage08RapidNaming.tsx — Stage 08: Rapid Automatized Naming (RAN - Speed Mountain)
 *
 * Measures:
 *   - Visual-verbal processing automaticity, lexical retrieval speed, and alphanumeric naming
 *   - Captures item latency as a continuous speed variable (latency_cv & mean latency)
 *
...
import { STAGE_08_ITEMS, type AssessmentCorpusItem } from '@/data/assessmentCorpus';
...
export function Stage08RapidNaming({ onStageComplete }: Stage08Props) {
  ...
  const [queue, setQueue] = useState<AssessmentCorpusItem[]>(() => {
    const practice = STAGE_08_ITEMS.find((i) => i.isPractice);
    const scoredMedium = STAGE_08_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'medium');
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (evalResult.shouldEscalateToHard) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_08_ITEMS, 'hard'));
      setActiveTier('hard');
    } else if (evalResult.shouldBranchToEasy) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_08_ITEMS, 'easy'));
      setActiveTier('easy');
    } else if (queue.length - (activeItemIndex + 1) + evalResult.nextValidScoredCount < DEFAULT_ADAPTIVE_CONFIG.minScoredItems) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_08_ITEMS, activeTier === 'hard' ? 'easy' : 'hard'));
    }

    // Advance quickly for rapid naming tasks (600ms)
    setTimeout(() => {
  ...
  if (!currentItem) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center">
        <p className="text-sm font-bold text-slate-700">Stage 08 complete!</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col w-full max-w-lg mx-auto px-4 py-5 gap-5 animate-fade-in" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-black text-slate-800">Island 8: Speed Mountain ⚡</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Replay audio speed prompt"
          >
            <Volume2 size={15} />
      ...
      <div
        ...
        style={{
          ...
          borderColor: currentItem.isPractice ? '#d8b4fe' : '#fde047',
        }}
      >
      ...
      {/* Prominent Stimulus Visual Card */}
      {currentItem.displayStimulus && (
        <div className="flex flex-col items-center justify-center py-2">
          <div className="w-24 h-24 rounded-3xl bg-amber-50 border-4 border-amber-300 shadow-md flex items-center justify-center animate-scale-in">
            <span className="text-5xl font-black text-amber-950">{currentItem.displayStimulus}</span>
          </div>
        </div>
      )}
