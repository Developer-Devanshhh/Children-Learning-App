/**
 * Stage07Spelling.tsx — Stage 07: Spelling & Orthographic Precision Screening
 *
 * Measures:
 *   - Orthographic precision, phoneme-to-grapheme encoding, and visual word memory
 *   - Distinguishes correct spellings from phonetically plausible and morphological distractors
 *
...
import { STAGE_07_ITEMS, type AssessmentCorpusItem } from '@/data/assessmentCorpus';
...
export function Stage07Spelling({ onStageComplete }: Stage07Props) {
  ...
  const [queue, setQueue] = useState<AssessmentCorpusItem[]>(() => {
    const practice = STAGE_07_ITEMS.find((i) => i.isPractice);
    const scoredMedium = STAGE_07_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'medium');
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (evalResult.shouldEscalateToHard) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_07_ITEMS, 'hard'));
      setActiveTier('hard');
    } else if (evalResult.shouldBranchToEasy) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_07_ITEMS, 'easy'));
      setActiveTier('easy');
    } else if (queue.length - (activeItemIndex + 1) + evalResult.nextValidScoredCount < DEFAULT_ADAPTIVE_CONFIG.minScoredItems) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_07_ITEMS, activeTier === 'hard' ? 'easy' : 'hard'));
    }
  ...
  if (!currentItem) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center">
        <p className="text-sm font-bold text-slate-700">Stage 07 complete!</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col w-full max-w-lg mx-auto px-4 py-5 gap-5 animate-fade-in" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-black text-slate-800">Island 7: Spell Forge ⚒️</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Replay audio spelling prompt"
          >
            <Volume2 size={15} />
            <span>Hear Word</span>
          </button>
      ...
      <div
        ...
        style={{
          background: currentItem.isPractice ? '#faf5ff' : '#fff7ed',
          borderColor: currentItem.isPractice ? '#d8b4fe' : '#fed7aa',
        }}
      >
