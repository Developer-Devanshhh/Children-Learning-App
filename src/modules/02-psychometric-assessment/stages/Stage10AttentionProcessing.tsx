/**
 * Stage10AttentionProcessing.tsx — Stage 10: Sustained Attention & Processing Speed (Star Focus)
 *
 * Measures:
 *   - Sustained visual search, selective feature attention, and inhibitory control against flankers
 *   - Captures processing reaction time, hit rate, and inhibition errors
 *
...
import { Volume2, CheckCircle2, Compass } from 'lucide-react';
import { Lyra } from '@/modules/04-attention-agent/Lyra';
import { audioEngine } from '@/modules/audio/audioEngine';
import { STAGE_10_ITEMS, type AssessmentCorpusItem } from '@/data/assessmentCorpus';
...
export function Stage10AttentionProcessing({ onStageComplete }: Stage10Props) {
  ...
  const [queue, setQueue] = useState<AssessmentCorpusItem[]>(() => {
    const practice = STAGE_10_ITEMS.find((i) => i.isPractice);
    const scoredMedium = STAGE_10_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'medium');
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (evalResult.shouldEscalateToHard) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_10_ITEMS, 'hard'));
      setActiveTier('hard');
    } else if (evalResult.shouldBranchToEasy) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_10_ITEMS, 'easy'));
      setActiveTier('easy');
    } else if (queue.length - (activeItemIndex + 1) + evalResult.nextValidScoredCount < DEFAULT_ADAPTIVE_CONFIG.minScoredItems) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_10_ITEMS, activeTier === 'hard' ? 'easy' : 'hard'));
    }
  ...
  if (!currentItem) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center">
        <p className="text-sm font-bold text-slate-700">Stage 10 complete!</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col w-full max-w-lg mx-auto px-4 py-5 gap-5 animate-fade-in" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-xl bg-teal-100 text-teal-700 flex items-center justify-center font-black text-xs">
            10
          </div>
          <div>
            <h2 className="text-sm font-black text-slate-800">Island 10: Star Focus 🔭</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Replay audio focus prompt"
          >
            <Volume2 size={15} />
      ...
      <div
        ...
        style={{
          background: currentItem.isPractice ? '#faf5ff' : '#f0fdfa',
          borderColor: currentItem.isPractice ? '#d8b4fe' : '#99f6e4',
        }}
      >
      ...
      {/* Target Stimulus Hint */}
      {currentItem.displayStimulus && (
        <div className="flex flex-col items-center justify-center py-2">
          <div className="px-6 py-3 rounded-2xl bg-teal-900 text-white border-2 border-teal-300 shadow-sm flex items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-wider text-teal-200">Target Target:</span>
            <span className="text-xl sm:text-2xl font-black">{currentItem.displayStimulus}</span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3.5 pt-2 flex-1 items-center">
        {currentItem.options.map((opt) => {
          ...
          let btnBg = 'white';
          let btnBorder = '2.5px solid #e2e8f0';
          ...
          return (
            <button
              ...
              className="min-h-[85px] p-4 rounded-3xl flex flex-col items-center justify-center gap-1 shadow-sm transition-all duration-100 active:scale-95 cursor-pointer touch-target hover:border-slate-300 disabled:cursor-not-allowed"
              style={{
                background: btnBg,
                border: btnBorder,
                color: textColor,
            ...
            >
              <span className="text-lg sm:text-xl font-black">{opt.label}</span>
              {isSelected && (
