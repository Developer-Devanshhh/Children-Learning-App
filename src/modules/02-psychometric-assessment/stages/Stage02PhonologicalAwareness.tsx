/**
 * Stage02PhonologicalAwareness.tsx — Stage 02: Phonological Awareness Screening
 *
 * Measures auditory manipulation of spoken sounds without reliance on print:
 *   - Rhyme Identification (e.g. Star -> Car)
 *   - Initial & Final Sound Isolation (e.g. Fish -> /f/, Cup -> /p/)
 *   - Phoneme Blending (e.g. /d/-/o/-/g/ -> Dog, /s/-/p/-/oo/-/n/ -> Spoon)
 *   - Phoneme Deletion (e.g. Park without /p/ -> Ark)
 *
...
import { STAGE_02_ITEMS, type AssessmentCorpusItem } from '@/data/assessmentCorpus';
...
export function Stage02PhonologicalAwareness({ onStageComplete }: Stage02Props) {
  ...
  const [queue, setQueue] = useState<AssessmentCorpusItem[]>(() => {
    const practice = STAGE_02_ITEMS.find((i) => i.isPractice);
    const scoredMedium = STAGE_02_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'medium');
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (evalResult.shouldEscalateToHard) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_02_ITEMS, 'hard'));
      setActiveTier('hard');
    } else if (evalResult.shouldBranchToEasy) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_02_ITEMS, 'easy'));
      setActiveTier('easy');
    } else if (queue.length - (activeItemIndex + 1) + evalResult.nextValidScoredCount < DEFAULT_ADAPTIVE_CONFIG.minScoredItems) {
      setQueue((prev) => ensureQueueSufficiency(prev, STAGE_02_ITEMS, activeTier === 'hard' ? 'easy' : 'hard'));
    }
  ...
  if (!currentItem) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center">
        <p className="text-sm font-bold text-slate-700">Stage 02 complete!</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col w-full max-w-lg mx-auto px-4 py-5 gap-5 animate-fade-in" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-black text-slate-800">Island 2: Sound River 🌊</h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Replay audio sound clue"
          >
            <Volume2 size={15} />
      ...
      {/* Lyra Audio Clue Prompt Card */}
      <div
        className="p-4 rounded-3xl border-2 flex items-start gap-3.5 shadow-sm"
        style={{
          background: currentItem.isPractice ? '#faf5ff' : '#f0f9ff',
          borderColor: currentItem.isPractice ? '#d8b4fe' : '#bae6fd',
        }}
      >
      ...
      {/* Options Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 pt-2 flex-1 items-center">
        {currentItem.options.map((opt) => {
          ...
          return (
            <button
              ...
              className="min-h-[85px] p-4 rounded-3xl flex flex-col items-center justify-center gap-1.5 shadow-sm transition-all duration-150 active:scale-95 cursor-pointer touch-target hover:border-slate-300 disabled:cursor-not-allowed"
              style={{
                background: btnBg,
                border: btnBorder,
                color: textColor,
