...
export function Stage01LetterKnowledge({ onStageComplete }: Stage01Props) {
  ...
  useEffect(() => {
    ...
    // Small delay before voice prompt for smooth screen transition
  ...
  const handleSelectOption = (option: AssessmentCorpusItem['options'][0]) => {
    ...
    if (!currentItem.isPractice) {
      ...
      if (consecutiveCorrectRef.current >= 2 && activeTier !== 'hard') {
        const hardItems = STAGE_01_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'hard');
      ...
      // Tier demotion: ≥2 consecutive incorrect → branch to Easy (PROVISIONAL)
        ...
        const easyItems = STAGE_01_ITEMS.filter((i) => !i.isPractice && i.difficultyTier === 'easy');
        setQueue((prev) => [...prev, ...easyItems.filter((e) => !prev.some((p) => p.id === e.id))]);
        setActiveTier('easy');
      }
    }

    // Snapshot ref values NOW (before the delay) to avoid stale-closure reads
    ...
    // Advance to next item or finish stage after friendly visual pause
    setTimeout(() => {
      // Ceiling rule: 3 consecutive correct at Hard tier → stop early (PROVISIONAL)
      const isCeilingMet = snapshotTier === 'hard' && snapshotCorrect >= 3;
      // Floor rule: 3 consecutive incorrect at Easy tier → stop early (PROVISIONAL)
      const isFloorMet = snapshotTier === 'easy' && snapshotIncorrect >= 3;
